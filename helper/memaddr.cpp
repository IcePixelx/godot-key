...
// Input  : vOpcodeArray - 
//-----------------------------------------------------------------------------
void CMemory::Patch(const std::vector<uint8_t> vOpcodeArray, bool bShouldVirtualProtect) const
{
	DWORD oldProt = NULL;

	SIZE_T dwSize = vOpcodeArray.size();

	if (bShouldVirtualProtect)
...
// Output : CMemory
//-----------------------------------------------------------------------------
CMemory CMemory::FindPattern(const std::string& svPattern, const Direction searchDirect, const int opCodesToScan, const ptrdiff_t occurence) const
{
...
// Output : CMemory
//-----------------------------------------------------------------------------
CMemory CMemory::FindPatternSelf(const std::string& svPattern, const Direction searchDirect, const int opCodesToScan, const ptrdiff_t occurence)
{
	uint8_t* pScanBytes = reinterpret_cast<uint8_t*>(ptr); // Get the base of the module.
